...
void ViewContext::Init(KVParser& config, int argc, char* argv[],
	...
	if (config["robot_cam"] != "") {
		...
		stringstream s;
		s
				<< "4 4 0.9786 0.2059 -0.0030 0 -0.2059 0.9781 -0.0289 0 -0.0030    0.0289    0.9996         0          0         0         0    1.0000";
	...
	// Copy in the cells.
	Get()._cells.clear();
	foreach(BareCell c, cells)Get()._cells.push_back(c);
...
void ViewContext::Draw() {
	...
	gluLookAt(TO[0][0], TO[1][0], TO[2][0], TF[0][0], TF[1][0], TF[2][0],
	...
	for (vector<BareCell>::iterator b = _cells.begin(); b != _cells.end();
			++b) {
		/*
...
int ViewContext::height() const {
	return _height;
}
int ViewContext::width() const {
	return _width;
}
...
void ViewContext::SetPoseFromConfig(KVParser& config, double x0, double y0,
		double z0) {
	// First step.
	...
	if (config["scanner"] == "" || config["camera"] == ""
			|| config["spin"] == "")
...
const vector<BareCell>& ViewContext::cells() const {
	return _cells;
}

ViewContext::ViewContext() :
		_width(0), _height(0), _robot(), _camera(), _cells(), _window(0), _poses(), _quit_requested(
				false) {
